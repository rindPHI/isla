...
def solve(parser, args):
    ...
    if all(not file.endswith(".bnf") for file in files):
    ...
    if all(not file.endswith(".isla") for file in files):
        parser.print_usage(file=sys.stderr)
        print(
...
Create solutions to an ISLa constraint and a reference grammar.""",
    ...
    parser.add_argument(
        "--grammar", help="the grammar in BNF (if not passed as a file)"
    )
    parser.add_argument(
        "--constraint", help="the ISLa constraint (if not passed as a file)"
    )
    parser.add_argument(
        "--output-dir", help="a directory into which to place generated output files"
    )
    parser.add_argument(
        "-n",
        "--num-solutions",
