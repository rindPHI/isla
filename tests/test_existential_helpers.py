...
class TestExistentialHelpers(unittest.TestCase):
    ...
    def test_insert_lang_2(self):
        ...
        result = insert_tree(
            canonical(LANG_GRAMMAR), inserted_tree, into_tree, max_num_solutions=30
        )
        ...
        self.assertTrue(
            all(t.find_node(into_tree.get_subtree((0, 2, 0))) for t in result)
        )
    ...
    def test_insert_lang_3(self):
        ...
        tree = DerivationTree(
            "<rhs>", (DerivationTree("<var>", None, id=12251),), id=12252
        )
    ...
    def test_insert_json_1(self):
        ...
        to_insert = DerivationTree.from_parse_tree(
            parse(' "key" : { "key" : null } ', JSON_GRAMMAR, "<member>")
        )

        results = insert_tree(
            canonical(JSON_GRAMMAR), to_insert, tree, max_num_solutions=20
        )
    ...
    def test_insert_json_2(self):
        ...
        tree = DerivationTree.from_parse_tree(parse(inp, JSON_GRAMMAR))
        to_insert = DerivationTree.from_parse_tree(
            parse(' "cheese" ', JSON_GRAMMAR, "<element>")
        )

        results = insert_tree(
            canonical(JSON_GRAMMAR), to_insert, tree, max_num_solutions=10
        )
    ...
    def test_insert_assignment_2(self):
            ...
            (
                DerivationTree(
                    ...
                    (
                        ...
                        DerivationTree(" ; ", (), id=5),
                        DerivationTree(
                            "<stmt>", (DerivationTree("<assgn>", id=7),), id=6
                        ),
                    ),
                    id=3,
                ),
        ...
        results = insert_tree(canonical(LANG_GRAMMAR), tree, in_tree, methods=methods)
    ...
    def test_tree_insert_direct_embedding(self):
        in_tree = DerivationTree(
            "<start>", (DerivationTree("<stmt>", None, id=0),), id=1
        )
        tree = DerivationTree("<assgn>", id=2)
        results = insert_tree(
            canonical(LANG_GRAMMAR), tree, in_tree, methods=DIRECT_EMBEDDING
        )
        self.assertTrue(
            all(
                result.find_node(node.id) is not None
                for result in results
                for _, node in result.paths()
            )
        )
    ...
    def test_insert_trees_assignment(self):
        ...
        self_embedding_tree = DerivationTree(
            "<stmt>",
            (
                DerivationTree("<assgn>", None),
                DerivationTree(" ; ", ()),
    ...
    def test_insert_xml_1(self):
        tree = DerivationTree.from_parse_tree(
            next(EarleyParser(XML_GRAMMAR).parse("<b>asdf</b>"))
        )
        to_insert = DerivationTree(
            "<xml-open-tag>",
            [
                DerivationTree("<", []),
                DerivationTree(
                    "<id>",
                    [DerivationTree("<id-start-char>", [DerivationTree("a", [])])],
                ),
                DerivationTree(">", []),
            ],
        )
    ...
    def test_insert_xml_2(self):
        ...
        result = insert_tree(
            canonical(XML_GRAMMAR_WITH_NAMESPACE_PREFIXES), to_insert, tree
        )
        self.assertTrue(result)

        str_results = [str(t) for t in result]
        self.assertIn(
            "<<id> <xml-attribute>><<id-no-prefix>:<id-no-prefix>/></<id>>", str_results
        )
    ...
    def test_insert_trees_xml_3(self):
                                ...
                                DerivationTree(
                                    ...
                                    (
                                        DerivationTree(
                                            ...
                                            (
                                                DerivationTree(
                                                    "<id-no-prefix>", None, id=13810
                                                ),
                                                DerivationTree(":", (), id=13811),
                                                DerivationTree(
                                                    "<id-no-prefix>", None, id=13812
                                                ),
                                            ),
                                            id=13602,
                                        ),
        ...
        self.assertTrue(
            all(
                result.find_node(node) is not None
                for result in results
                for _, node in to_insert.paths()
            )
        )
        self.assertTrue(
            all(
    ...
    def test_insert_xml_4(self):
                    ...
                    (
                        DerivationTree(
                            ...
                            (
                                ...
                                DerivationTree("<id-no-prefix>", None, id=139915),
                            ),
                            id=139889,
                        ),
                    ),
        ...
        into_tree = DerivationTree(
            ...
            (
                DerivationTree(
                    "<id>",
                    (DerivationTree("<id-no-prefix>", None, id=138999),),
                    id=137339,
                ),
                DerivationTree('="', (), id=137340),
                DerivationTree("<text>", None, id=137341),
                DerivationTree('"', (), id=137342),
            ),
            id=21903,
        )
